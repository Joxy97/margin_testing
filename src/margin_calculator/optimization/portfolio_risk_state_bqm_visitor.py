...
from risk_state_generator.risk_state import (
...
class _StructuralQUBOTemplate:
    ...
    def __post_init__(self):
        for name in ("offsets", "linear", "heads", "tails", "biases"):
            value = getattr(self, name)
            object.__setattr__(self, name, numpy.frombuffer(value.tobytes(), dtype=value.dtype).reshape(value.shape))
...
class PortfolioRiskStateBQMVisitor:
    ...
    def _createBQM(
        ...
        template = self.oneHotTopology(state_counts, lambda_one_hot)
        ...
        return QUBOProblem(
            ...
            seedOffset=self.scenarioSeedOffset(
                instruments,
                linear,
                correlations,
                lambda_one_hot,
    ...
    def oneHotTopology(self, stateCounts: tuple[int, ...], lambdaOneHot: float) -> _StructuralQUBOTemplate:
        ...
        template_key = (stateCounts, lambdaOneHot)
        template = self.structuralCache.get(template_key)
        if template is None:
            offsets = numpy.empty(len(stateCounts) + 1, dtype=numpy.int64)
            offsets[0] = 0
            numpy.cumsum(stateCounts, out=offsets[1:])
            variable_count = int(offsets[-1])
            one_hot_heads = []
            one_hot_tails = []
            triangle_cache = {}
            for asset, count in enumerate(stateCounts):
            ...
            template = _StructuralQUBOTemplate(
                ...
                offset=lambdaOneHot * len(stateCounts),
            )
    ...
    _stableSeedOffset = scenarioSeedOffset  # Compatibility for existing Python callers.
