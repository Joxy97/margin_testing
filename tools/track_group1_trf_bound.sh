...
REMOTE=/workspace/group1-trf-bound-20260910/experiments/group1_trf_bound_20260910/results
LOCAL="$ROOT/experiments/group1_trf_bound_20260910/results"
...
case "${1:-status}" in
  status) ssh "${OPTIONS[@]}" "$HOST" "supervisorctl status group1_trf_bound; cat $REMOTE/status.json" ;;
  ...
  fetch)
    ...
    ;;
  *) printf 'Usage: bash tools/track_group1_trf_bound.sh [status|logs|fetch]\n' >&2; exit 2 ;;
esac
